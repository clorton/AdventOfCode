...
def foo(counts):
    return reduce(lambda a, b: a + chr(65+b), counts, '')
...
while state not in seen:
    # print('Adding {0} to seen states.'.format(state))
    seen.add(state)
    index = locate(memory)
    distribute(memory, index)
    state = foo(memory)

print('{0} cycles'.format(len(seen)))
...
print("State '{0}', initially seen on cycle {1} re-encountered on cycle {2}".format(state, seen[state], count))
print('Loop length {0}'.format(count - seen[state]))

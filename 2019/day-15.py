...
DELTAS = [None, Delta(0, -1), Delta(0, 1), Delta(-1, 0), Delta(1, 0)]
RIGHT = [None, Direction.EAST, Direction.WEST, Direction.NORTH, Direction.SOUTH]
LEFT = [None, Direction.WEST, Direction.EAST, Direction.SOUTH, Direction.NORTH]
...
def follow_right(_program):

    remote = Computer(_program, [])
    walls = set()
    space = {Point(0, 0): 0}
    x, y, direction = 0, 0, Direction.NORTH
    oxygen = None
    distance = 0
    while True:
        output = Output(remote.run([int(direction)]))
        if output == Output.MOVE:
            x += DELTAS[int(direction)].dx
            y += DELTAS[int(direction)].dy
            direction = RIGHT[int(direction)]
            here = Point(x, y)
            distance += 1
            if here in space:
                distance = min(distance, space[here])
            else:
                space[here] = distance
            # if x == 0 and y == 0:
            #     raise RuntimeError("Returned to starting position.")
        elif output == Output.WALL:
            walls.add(Point(x + DELTAS[int(direction)].dx, y + DELTAS[int(direction)].dy))
            direction = LEFT[int(direction)]
        elif output == Output.OXYGEN:
            x += DELTAS[int(direction)].dx
            y += DELTAS[int(direction)].dy
            oxygen = Point(x, y)
            distance += 1
    ...
    return walls, space, oxygen, distance


def follow_left(prog):

    remote = Computer(prog, [])
    walls = set()
    space = {Point(0, 0): 0}
    x, y, direction = 0, 0, Direction.NORTH
    oxygen = None
...
def extents(walls):
    ...
    min_x = reduce(lambda m, p: min(m, p.x), walls, 1 << 31)
    min_y = reduce(lambda m, p: min(m, p.y), walls, 1 << 31)
    max_x = reduce(lambda m, p: max(m, p.x), walls, -(1 << 31))
    max_y = reduce(lambda m, p: max(m, p.y), walls, -(1 << 31))

    return min_x, min_y, max_x, max_y


def local_map(walls, space=None, oxygen=None, center=True):

    min_x, min_y, max_x, max_y = extents(walls)

    if oxygen is not None:
        min_x = min(min_x, oxygen.x)
        min_y = min(min_y, oxygen.y)
        max_x = max(max_x, oxygen.x)
        max_y = max(max_y, oxygen.y)

    width = max_x - min_x + 1
    height = max_y - min_y + 1

    dungeon = [[" " for _ in range(width)] for __ in range(height)]

    for point in walls:
        dungeon[point.y-min_y][point.x-min_x] = "#"

    if space is not None:
        for point, dist in space.items():
            dungeon[point.y-min_y][point.x-min_x] = "0123456789"[dist % 10]

    if center:
        dungeon[0-min_y][0-min_x] = "."

    if oxygen is not None:
        dungeon[oxygen.y-min_y][oxygen.x-min_x] = "@"

    return dungeon


def display(walls, space=None, oxygen=None, distance=0):

    dungeon = local_map(walls, space, oxygen)

    for row in dungeon:
        print(''.join(row))

    if oxygen is not None:
        print(f"Found oxygen at point {oxygen} after {distance} moves.")
...
def main():
    ...
    walls_right, space_right, oxygen_right, distance_right = follow_right(program)
    display(walls_right, space_right, oxygen_right, distance_right)

    print("\n")

    walls_left, space_left, oxygen_left, distance_left = follow_left(program)
    display(walls_left, space_left, oxygen_left, distance_left)

    print("\n")

    all_walls = walls_right.union(walls_left)
    # display(all_walls)

    # Part 2

    initial = set()
    initial.add(oxygen_right)
    fill = [initial]
    filled = set()
    filled.add(oxygen_right)
    min_x, min_y, max_x, max_y = extents(all_walls)
    while len(fill[-1]) > 0:

        # print(f"After {len(fill)-1} minutes...")
        # display(all_walls, {p: len(fill) for p in fill[-1]})
        # print("\n")

        subsequent = set()
        for point in fill[-1]:
            filled.add(point)
            for adjacent in [Direction.NORTH, Direction.SOUTH, Direction.EAST, Direction.WEST]:
                test = Point(point.x + DELTAS[adjacent].dx, point.y + DELTAS[adjacent].dy)
                if (test.x >= min_x) and (test.x <= max_x) and (test.y >= min_y) and (test.y <= max_y) and \
                        (test not in filled) and (test not in all_walls):
                    subsequent.add(test)
        fill.append(subsequent)

    # Spaces in fill[i] are filled after i minutes.
    # fill[-1] == fill[len(fill)-1] is empty, so fill[len(fill)-2] has the last space(s) to fill.
    # Those spaces are filled after len(fill)-2 minutes (see first point).
    print(f"Map is filled after {len(fill)-2} minutes.")
